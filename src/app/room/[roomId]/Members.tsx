    ...
    const columns = [
        {
            ...
            render: (row: Member) => (
                <div className="flex items-center gap-2 w-full">
                    <p>{row.name}</p>
                    {row.role === "admin" && <span className="bg-primary/10 border border-primary text-primary px-2 rounded text-xs">Admin</span>}
                    {row.id === localMember.id && <span className="bg-green-600/10 border border-green-600 text-green-600 px-2 rounded text-xs">You</span>}
                </div>
            )
        },
        ...isAdmin ?
            [{
                label: "Actions",
                key: "actions",
                render: (row: Member) => row.role === 'admin' ? null : (
                    <Button onClick={() => { setOpen(true); setSelected(row) }} variant={"destructive"}>
                        Remove
                    </Button>
                )
            }] : [],
            ...
            <AlertDialog open={open} onOpenChange={setOpen}>
                <AlertDialogContent>
                    ...
                    <AlertDialogFooter>
                        ...
                        <AlertDialogAction onClick={() => handleRemoveMember(selected?.id!)}>Yes, Remove</AlertDialogAction>
                    </AlertDialogFooter>
                </AlertDialogContent>
            </AlertDialog>
        </div>
